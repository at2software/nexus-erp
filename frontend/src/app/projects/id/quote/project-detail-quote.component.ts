...
export class ProjectDetailQuoteComponent {
    ...
    constructor() {
        effect(() => {
            ...
            for (const o of object.assignees) {
                if (o.company_contact_id) {
                    const m = Assignee.fromJson(o);
                    this.personalized = m.assignee;
                }
            }
            this.quoteDescriptions = object.quote_descriptions || [];
        });
    }
    ...
    #iterateItems = (fn: (x: InvoiceItem) => void) => this.parent.object()?.invoice_items.forEach(fn);
}
