...
import { Dictionary, filtered } from '@constants/constants';
...
export class ProjectStateFilterComponent {
    ...
    stateFilters = signal<Project[]>([this.filters().prepared, this.filters().inProgress, this.filters().finished, this.filters().failed, this.filters().alternative]);
    #filterIconsInternal = signal<Project[]>([this.filters().internal, this.filters().external]);
    #filterIconsBudgeting = signal<Project[]>([this.filters().timeBased, this.filters().budgetBased]);
    ...
    constructor() {
        ...
        if (cookieData) {
            ...
            if (parsed) {
                const [states, budgeting, internal] = parsed;
    ...
    isHidden(p: Project): boolean {
        ...
        return this.#filterWillHide(p, this.filters().prepared, ['state']) || this.#filterWillHide(p, this.filters().inProgress, ['state']) || this.#filterWillHide(p, this.filters().finished, ['state', 'finished_state']) || this.#filterWillHide(p, this.filters().failed, ['state', 'finished_state']) || this.#filterWillHide(p, this.filters().alternative, ['state', 'finished_state']) || this.#filterWillHide(p, this.filters().budgetBased, ['is_time_based']) || this.#filterWillHide(p, this.filters().timeBased, ['is_time_based']) || this.#filterWillHide(p, this.filters().internal, ['is_internal']) || this.#filterWillHide(p, this.filters().external, ['is_internal']);
    }

    onValueChanged() {
        setCookie(this.cookieId(), JSON.stringify([this.stateFilters().map((_) => _.var.enabled), this.#filterIconsBudgeting().map((_) => _.var.enabled), this.#filterIconsInternal().map((_) => _.var.enabled)]), 7);
        this.filterChanged.emit();
    }

    #enableAllIfRequired() {
        if (this.defaultEnableAll()) {
            [...this.stateFilters(), ...this.#filterIconsBudgeting(), ...this.#filterIconsInternal()].forEach((_) => (_.var.enabled = true));
            this.filterChanged.emit();
        }
    }
}
