...
export class ProfileSentinelTriggerModalComponent extends ModalBaseComponent<boolean> {
    ...
    #isExcludedTable(tableName: string): boolean {
        const excluded = ['assignment_roles', 'encryptions', 'password_reset_tokens', 'personal_access_tokens', 'telescope_entries', 'telescope_entries_tags', 'telescope_monitoring', 'user_employments', 'user_paid_times', 'vaults', 'websockets_statistics_entries'];
    ...
    getTriggerColor(trigger: number): string {
        switch (trigger) {
            ...
            case ObserverTrigger.OnDeleted:
                return 'danger';
            case ObserverTrigger.OnSchedule:
                return 'cyan';
            default:
                return 'dark-grey';
        }
