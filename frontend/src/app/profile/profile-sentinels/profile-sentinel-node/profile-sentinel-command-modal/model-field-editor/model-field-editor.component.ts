...
export class ModelFieldEditorComponent {
    ...
    parseFieldsJson() {
        ...
        this.fields.update((fields) => fields.map((f) => (values[f.field] !== undefined ? { ...f, enabled: true, value: values[f.field] } : f)));
    }
    ...
    onInputKeyup(event: KeyboardEvent, field: FieldValue) {
        ...
        switch (event.key) {
            case 'ArrowDown':
                this.suggestionIndex.update((i) => (i + 1) % Math.max(1, suggestions.length));
                break;
            case 'ArrowUp':
                this.suggestionIndex.update((i) => (suggestions.length + i - 1) % Math.max(1, suggestions.length));
                break;
            case 'Enter':
                if (suggestions.length) {
                    this.selectSuggestion(field, suggestions[this.suggestionIndex()]);
    ...
    getSuggestions(input: string, currentTable: string): { name: string; type: string }[] {
        ...
        const triggerVar = this.triggerVariable();
        const loopVar = this.loopVariable();
        const loopTable = this.loopTable();
        const variables = [...(triggerVar ? [{ name: triggerVar, table: currentTable }] : []), ...(loopVar && loopTable ? [{ name: loopVar, table: loopTable }] : []), { name: 'old', table: currentTable }];
