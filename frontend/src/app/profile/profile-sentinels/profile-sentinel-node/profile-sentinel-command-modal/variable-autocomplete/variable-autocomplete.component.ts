...
export class VariableAutocompleteComponent {
    ...
    onKeydown(event: KeyboardEvent) {
        ...
        switch (event.key) {
            ...
            case 'Enter':
                event.preventDefault();
                this.selectResult(results[this.currentIndex()]);
                break;
            case 'Escape':
                this.results.set([]);
                break;
    ...
    getSuggestions(input: string): Suggestion[] {
        ...
        const variables = [...(triggerVar ? [{ name: triggerVar, table: this.tableName() }] : []), ...(loopVar && loopTable ? [{ name: loopVar, table: loopTable }] : []), { name: 'old', table: this.tableName() }];
