...
export class MantisSubPlugin extends MantisPlugin {
    ...
    indexTasks = (filterId?: string): Observable<Task[]> => {
        ...
        const pageSize = 100;
        return this.fetchAllPages((page) => this.aget(`issues?${queryParams}&page_size=${pageSize}&page=${page}`, {}, this.toTask) as unknown as Observable<Task[]>, pageSize);
    };
    indexOpenTasks = (): Observable<Task[]> => this.aget(`issues?filter_id=assigned`, {}, this.toTask) as unknown as Observable<Task[]>;
    ...
    indexTasksPage = (page: number, pageSize: number): Observable<{ tasks: Task[]; hasMore: boolean }> => {
        ...
        return (this.aget(`issues?${queryParams}&page_size=${pageSize}&page=${page}`, {}, this.toTask) as unknown as Observable<Task[]>).pipe(map((tasks) => ({ tasks, hasMore: tasks.length >= pageSize })));
    };
