...
export class GitService {
    ...
    initAll = () =>
        new Promise<void>((resolve) => {
            ...
            this.#instances = this.getRepositories().map((_) => new GitServiceInstance(_ as Dictionary));
    ...
    #initInstance = async (i: GitServiceInstance): Promise<void> => {
        try {
            const result = await firstValueFrom(this.#http.get<Dictionary>(this.#urlFor(i, 'user'), i.headers()));
            i.isAvailable = true;
    ...
    get = <T = unknown>(path: string, data: Dictionary<string> | undefined, instance: GitServiceInstance, rawUrl?: boolean) =>
    ...
    static pipeToUser = (_: Dictionary): IUser => ({ id: _['id'] as string, name: _['name'] as string });
}
...
export class GitServiceInstance {
    ...
    constructor(encData: Dictionary) {
        this.url = String(encData.url ?? '');
        this.token = String(encData.token ?? '');
    }
    headers = () => ({
