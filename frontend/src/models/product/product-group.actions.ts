...
export function getProductGroupActions(self: ProductGroup): NxAction[] {
    return [
        ...
        { title: $localize`:@@i18n.products.setActive:setActive`, on: () => !self.is_active, group: true, action: () => self.put('activate') },
        {
            ...
            interrupt: { service: ModalConfirmComponent, args: { message: $localize`:@@i18n.products.reallyDeleteThisProductGroup:really delete this product group?`, title: $localize`:@@i18n.common.attention:attention` } },
            action: () => self.delete(),
