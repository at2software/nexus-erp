...
export class VCalendarEvent {
    ...
    static extractEventData(vcalString: string, isEditable: boolean = false): VCalendarEvent {
        ...
        const dtstartResult = this.extractDate(vcalString, 'DTSTART') ?? this.extractDate(vcalString, 'VALUE=DATE') ?? { date: new Date(), entireDay: false };
    ...
    static parseDateString(dateString: string): { date: Date; entireDay: boolean } | null {
        ...
        if (dateFormat1.test(dateString)) {
            ...
            const date = new Date(Date.UTC(parseInt(dateString.substring(0, 4), 10), parseInt(dateString.substring(4, 6), 10) - 1, parseInt(dateString.substring(6, 8), 10), parseInt(dateString.substring(9, 11), 10), parseInt(dateString.substring(11, 13), 10), parseInt(dateString.substring(13, 15), 10)));
            return { date: date, entireDay: false };
        } else if (dateFormat3.test(dateString)) {
            const date = new Date(parseInt(dateString.substring(0, 4), 10), parseInt(dateString.substring(4, 6), 10) - 1, parseInt(dateString.substring(6, 8), 10), parseInt(dateString.substring(9, 11), 10), parseInt(dateString.substring(11, 13), 10), parseInt(dateString.substring(13, 15), 10));
            return { date: date, entireDay: false };
        } else {
            return null;
        }
