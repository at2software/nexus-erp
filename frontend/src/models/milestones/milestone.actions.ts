...
export function getMilestoneActions(self: Milestone): NxAction[] {
    return [
        ...
        },
        {
            ...
            group: true,
            on: () => (NxGlobal.context instanceof Milestone && NxGlobal.context.project && NxGlobal.context.project.assignedUsers().length > 0) || false,
        ...
        },
        {
            ...
            action: () => {
                ...
                inputModal
                    .open($localize`:@@i18n.milestone.workloadHours:workload (hours)`, false, $localize`:@@i18n.milestone.setDurationInfo:Enter the estimated workload in hours for this milestone`, initialValue)
                    .then((result) => {
