...
export class ProjectService extends NexusHttpService<Project> {
    ...
    index = (filters?: Dictionary): Observable<Project[]> => {
        return this.paginate(this.apiPath, filters);
    };
    indexForCompany = (company: Company, filters?: Dictionary): Observable<Project[]> => {
        return this.aget(`companies/${company.id}/projects`, filters);
    };
    indexCoParticipatedProjects = (company: Company, filters?: Dictionary): Observable<Project[]> => {
        return this.aget(`companies/${company.id}/co-participated-projects`, filters);
    };
