...
export class ProjectAvatar {
    ...
    static BudgetBased = (args: any = {}) => this.Running({ is_time_based: 0, ...args });
    static TimeBased = (args: any = {}) => this.Running({ is_time_based: 1, ...args });
    static Internal = (args: any = {}) => this.Running({ is_internal: 1, ...args });
    static External = (args: any = {}) => this.Running({ is_internal: 0, ...args });
}
...
export class ProjectState extends Serializable {
    ...
    static idsFor = (progress: number) =>
        NxGlobal.global.project_states
            .filter((_) => _.progress == progress)
            .map((_) => _.id)
            .join(',');
    static idsPrepared = () =>
        NxGlobal.global.project_states
            .filter((_) => _.isPrepared())
            .map((_) => _.id)
            .join(',');
    static idsRunning = () =>
        NxGlobal.global.project_states
            ...
            .map((_) => _.id)
            .join(',');
    static idsSuccessful = () =>
        NxGlobal.global.project_states
            .filter((_) => _.isFinishedSuccessful())
            .map((_) => _.id)
            .join(',');
    static idsFailed = () =>
        NxGlobal.global.project_states
            .filter((_) => _.isFinishedFailed())
            .map((_) => _.id)
            .join(',');
    static idsIgnored = () =>
        NxGlobal.global.project_states
            .filter((_) => _.isFinishedIgnored())
            .map((_) => _.id)
            .join(',');
